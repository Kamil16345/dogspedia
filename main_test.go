...
func TestGetDogHandler(t *testing.T){
	dogs=[]Dog{
		{"Labrador retriever", "Medium-size dog"},
	}
	req, err := http.NewRequest("GET", "", nil)
	if err != nil{
		t.Fatal(err)
	...
	recorder := httptest.NewRecorder()
	hf := http.HandlerFunc(getDogHandler)
	hf.ServeHTTP(recorder, req)

	if status := recorder.Code; status!= http.StatusOK{
		t.Errorf("handler returned wrong status code, get %v, want %v", status, http.StatusOK)
	}
	expected := Dog{"Labrador retriever", "Medium-size dog"}
	b := []Dog{}
...
func TestCreateDogsHandler(t *testing.T){
	dogs = []Dog{
		{"Labrador retriever", "Medium-size dog"},
	}
	form := newCreateDogForm()
	...
	hf := http.HandlerFunc(createDogHandler)
	hf.ServeHTTP(recorder, req)
...
func newCreateDogForm() *url.Values{
	form := url.Values{}
	form.Set("species", "eagle")
	form.Set("description", "Medium-size dog")
	return &form
}